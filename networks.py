...
class RNN(nn.Module):
    """
    An RNN module with a linear input layer, RNN layer, linear output layer that is used for the
    different stages of prediction
    """
    def __init__(self, num_inputs, num_outputs, hidden_size, rnn_layers=2, bidirectional=True, dropout=0.2):
        super(RNN, self).__init__()
        self.rnn = nn.LSTM(hidden_size, hidden_size, rnn_layers, bidirectional=bidirectional)
        self.linear1 = nn.Linear(num_inputs, hidden_size)
        self.linear2 = nn.Linear(hidden_size * (2 if bidirectional else 1), num_outputs)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, h=None):
        x, h = self.rnn(torch.relu(self.linear1(self.dropout(x))).unsqueeze(1), h)
        return self.linear2(x.squeeze(1)), h


class Transpose(nn.Module):
    """
    The whole Transpose pipeline
    """
    def __init__(self,  num_past_frame=20, num_future_frame=5, hip_length=None, upper_leg_length=None,
        ...
        """
        :param num_past_frame: Number of past frames
        :param num_future_frame: Number of future frames
        :param hip_length: Hip length in meters. SMPL mean length is used by default
        :param upper_leg_length: Upper leg length in meters. SMPL mean length is used by default.
        :param lower_leg_length: Lower leg length in meters. SMPL mean length is used by default.
        :param prob_threshold: Probability threshold to control the fusion of the two translation branches
        :param gravity_velocity: Gravity velocity added to trans-B1 when the body is off the ground
        """
        super().__init__()
        ...
        self.pose_b1 = RNN(joint_set.n_leaf * 3 + n_imu, 2, 64)
        self.pose_b2 = RNN(joint_set.n_full * 3 + n_imu, 3, 256, bidirectional=False)
        ...
        self.last_root_pose = torch.zeros(3)
        self.reset()
