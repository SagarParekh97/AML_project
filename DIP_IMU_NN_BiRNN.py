...
def train(basepath, args):
	...
	f.close()
	pickle.dump(train_loss, open(basepath + 'train_loss.pkl'))
	pickle.dump(valid_loss, open(basepath + 'validation_loss.pkl'))
...
def plotGraph(train_loss,val_loss,basepath):
	...
	fig = plt.figure(1)
	plt.plot(train_loss.item(), 'r--', label='training loss')
	plt.plot(val_loss.item(), 'g--', label = 'validation loss')
	plt.legend()
